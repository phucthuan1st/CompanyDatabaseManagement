...
BEGIN
    ...
    IF p_VAITRO = 'Nhân viên' THEN
        EXECUTE IMMEDIATE 'GRANT COMPANY_PUBLIC.NHAN_VIEN TO ' || p_MANV;
    ELSIF p_VAITRO = 'QL trực tiếp' THEN
        EXECUTE IMMEDIATE 'GRANT COMPANY_PUBLIC.QL_TRUC_TIEP TO ' || p_MANV;
    ELSIF p_VAITRO = 'Trưởng phòng' THEN
        EXECUTE IMMEDIATE 'GRANT COMPANY_PUBLIC.TRUONG_PHONG TO ' || p_MANV;
    ELSIF p_VAITRO = 'Tài chính' THEN
        EXECUTE IMMEDIATE 'GRANT COMPANY_PUBLIC.TAI_CHINH TO ' || p_MANV;
    ELSIF p_VAITRO = 'Nhân sự' THEN
        EXECUTE IMMEDIATE 'GRANT COMPANY_PUBLIC.NHAN_SU TO ' || p_MANV;
    ELSIF p_VAITRO = 'Trưởng đề án' THEN
        EXECUTE IMMEDIATE 'GRANT COMPANY_PUBLIC.TRUONG_DEAN TO ' || p_MANV;
    ELSE 
        ROLLBACK;
    END IF;
